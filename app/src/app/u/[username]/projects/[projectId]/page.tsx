              ...
              {project.project_repos.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {project.project_repos.map((r) => {
                    const isGitHub = r.repo_url.includes("github.com");
                    return (
                      <a
                        key={r.id}
                        href={r.repo_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 rounded-full bg-[#f5f0e8] px-3 py-1 text-sm text-[#78716c] hover:underline"
                      >
                        {isGitHub && (
